...
class ChatBloc extends Bloc<ChatEvent, ChatState> {
  ...
  FutureOr<void> individualEvent(
      IndividualEvent event, Emitter<ChatState> emit) {
    emit(NavigateToIndividualState(user: event.user));
  }

  FutureOr<void> tesxtMessagesentEvent(
      TesxtMessagesentEvent event, Emitter<ChatState> emit) {
    Chat chat = Chat();
    chat.creatChatRoom(event.receiverId, event.text, "text");
  }

  FutureOr<void> emojiCoditionEvent(
      EmojiCoditionEvent event, Emitter<ChatState> emit) {
    emit(EmojiCoditionState(isEmoji: event.isEmoji));
  }

  FutureOr<void> imagePickEvent(
      ImagePickEvent event, Emitter<ChatState> emit) async {
    Chat chat = Chat();
    // media pickig
    List<XFile?> images = await ImagePicker().pickMultipleMedia();
    if(images.isNotEmpty&&images!=null){
       emit(await ImageloadingState(sentImage: images.first!.path));
    }
    List<String> _imgUrl = [];
    for (int i = 0; i < images.length; i++) {
      ...
      if (images[i]!.path.contains("jpg")) {
        _imgUrl.add(
            await ChatImageUpload("Chatmedia", await images[i]!.readAsBytes()));
            
      } else {
        _imgUrl.add(
            await ChatvideoUpload("Chatmedia", await images[i]!.readAsBytes()));
        log(images[0]!.path);
      }
    }
  ...
  FutureOr<void> cameraPickEvent(
      CameraPickEvent event, Emitter<ChatState> emit) async {
