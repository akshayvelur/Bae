                    ...
                    child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                      ...
                      builder: (BuildContext context,
                        ...
                        int numberOfUser = 0;
                           ...
                           //distance filetr
                           List<Map<String,dynamic>>distanceFinfilter=genderfilter.where((element) => distancefilterd.contains(element['uid']),).toList();
                         //liked filter
                         List<Map<String,dynamic>>likedFilterd=distanceFinfilter.where((element) =>!likeList.contains(element['uid']) ,).toList();
                         //age filter
                           List<Map<String, dynamic>> filteredList =likedFilterd.where((element) {
                            int age = currentyear -
                                int.parse(element['dob'].split("/").last);
                          return age>= int.parse(ageRange[0])&&age<=int.parse(ageRange[1]);   
                         },).toList();
